    ...
    public void test() throws Exception {
        ...
        try (Closer closer = Closer.create()) {
            try {
                ...
                Path rdfXmlFile = null;
                if (loadXml) {
                    Path file = Files.createTempFile("qlever-test-", ".rdf");
                    closer.register(() -> Files.delete(file));
                ...
                RDFDatabaseBuilder<?> databaseBuilder = loader.newBuilder()
                    .setOutputFolder(tmpDbFolder)
                    .addPath(ntFile.toString());
                if (loadXml) {
                    databaseBuilder.addPath(rdfXmlFile.toString());
                }
                RDFDatabase database = databaseBuilder.build();
