 ...
 */
public class TrackingApplyTransformVisitor implements OpVisitorByTypeAndExpr, ExprVisitor {
    ...
    protected Expr transform(Expr expr) {
        ...
        try {
            ...
            if ( x1 != y1 )
                Log.error(TrackingApplyTransformVisitor.class, "Misaligned opStack") ;
            if ( x2 != y2 )
                Log.error(TrackingApplyTransformVisitor.class, "Misaligned exprStack") ;
        }
    }
...
//            return exprList ;
    ...
    private <T> void push(Deque<T> stack, T value) {
        if ( value == null )
            Log.warn(TrackingApplyTransformVisitor.class, "Pushing null onto the "+stackLabel(stack)+" stack") ;
        stack.push(value) ;
    }
    ...
    private <T> T pop(Deque<T> stack) {
        try {
            T v = stack.pop() ;
            if ( v ==  null )
                Log.warn(TrackingApplyTransformVisitor.class, "Pop null from the "+stackLabel(stack)+" stack") ;
            return v ;
        }
        catch (NoSuchElementException ex) {
            if ( true ) throw new RuntimeException() ;
            Log.warn(TrackingApplyTransformVisitor.class, "Empty "+stackLabel(stack)+" stack") ;
            return null ;
        }
    }
